...
Install {BiocManager} and {edgeR}; fails because `there is no package called ‘statmod’`.

```{r bioc_manager_edger, eval=FALSE}
install.packages("BiocManager")
BiocManager::install("edgeR")
renv::snapshot()
```

Installing {SingleCellExperiment} also doesn't work:

```{r bioc_manager_sce, eval=FALSE}
